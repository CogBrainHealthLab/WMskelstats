...
#' @param data A numeric vector of overlay values mapped to each 3D coordinate point.
...
#' @importFrom graphics par plot polygon abline grid points image axis box mtext
...
plot_skel <- function(coords, data, template = rep(NA, length(data))) {
  ...
  ui <- fluidPage(
    sidebarLayout(
      sidebarPanel(
        ...
        h4("Data Controls"),
        sliderInput("lower_thresh_val", "Lower threshold:", min = round(thresh_min, 2), max = round(thresh_max, 2), value = round(thresh_min, 2), step = round(thresh_step, 2)),
  ...
  server <- function(input, output, session) {
    ...
    # Pre-initialize event ID to prevent Plotly's asynchronous onFlushed warning
    session$userData$plotlyShinyEventIDs <- unique(c(
      session$userData$plotlyShinyEventIDs, 
      "plotly_click-skeleton_3d"
    ))

    df_filtered <- reactive({
      req(input$lower_thresh_val, input$upper_thresh_val)
      df_mod <- df
    ...
    click_data <- reactive({
      suppressWarnings(plotly::event_data("plotly_click", source = "skeleton_3d"))
    })
    
    observeEvent(click_data(), {
      cd <- click_data()
      req(cd)
      if (!is.null(cd$x)) updateSliderInput(session, "x_slider", value = round(cd$x))
      if (!is.null(cd$y)) updateSliderInput(session, "y_slider", value = round(cd$y))
      if (!is.null(cd$z)) updateSliderInput(session, "z_slider", value = round(cd$z))
    ...
    output$plot_3d <- renderPlotly({
      ...
      if (has_grey) {
        p <- add_trace(p, x = d_grey$X, y = d_grey$Y, z = d_grey$Z, type = "scatter3d", mode = "markers", showlegend = FALSE,
                       marker = list(
      ...
      if (has_na) {
        p <- add_trace(p, x = d_na$X, y = d_na$Y, z = d_na$Z, type = "scatter3d", mode = "markers", showlegend = FALSE,
                       marker = list(
                         size = max(1, input$pt_size * 0.6),
                         color = d_na$tmpl,
      ...
      if (has_valid) {
        p <- add_trace(p, x = d_valid$X, y = d_valid$Y, z = d_valid$Z, type = "scatter3d", mode = "markers", name = "Data Overlay",
                       marker = list(
                         size = input$pt_size,
                         opacity = 0.85,
                         color = d_valid$val,
      ...
      p <- layout(p, title = list(text = "3D Skeleton Overlay", font = list(color = fg_3d)), 
                  paper_bgcolor = bg_3d, plot_bgcolor = bg_3d, autosize = TRUE,
    ...
    output$download_2d_png <- downloadHandler(
      filename = function() {
      ...
      content = function(file) {
        ...
        graphics::layout(matrix(c(1, 2, 3, 4, 4, 4), nrow = 2, byrow = TRUE), heights = c(4.2, 1))
